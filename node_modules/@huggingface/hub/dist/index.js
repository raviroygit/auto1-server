  ...
  "src/utils/FileBlob.ts"() {
    ...
    FileBlob = class extends Blob {
      ...
      static async create(path) {
        ...
        const { size } = await (0, import_promises2.stat)(path);
...
async function scanCachedRepo(repoPath) {
  ...
  const repoLastAccessed = blobStats.size > 0 ? Math.max(...[...blobStats.values()].map((stat4) => stat4.atimeMs)) : repoStats.atimeMs;
  const repoLastModified = blobStats.size > 0 ? Math.max(...[...blobStats.values()].map((stat4) => stat4.mtimeMs)) : repoStats.mtimeMs;
  return {
    id: {
      name: repoId,
      type: repoType
    ...
    size: [...blobStats.values()].reduce((sum, stat4) => sum + stat4.size, 0),
...
async function exists(path, followSymlinks) {
  try {
    if (followSymlinks) {
      ...
      await (0, import_promises3.lstat)(path);
    }
    return true;
  } catch (err) {
    return false;
...
async function downloadFileToCacheDir(params) {
  ...
  await (0, import_promises3.mkdir)((0, import_node_path2.dirname)(blobPath), { recursive: true });
  await (0, import_promises3.mkdir)((0, import_node_path2.dirname)(pointerPath), { recursive: true });
  if (await exists(blobPath)) {
    await (0, import_promises3.symlink)(blobPath, pointerPath);
    return pointerPath;
  }
  ...
  await (0, import_promises3.writeFile)(incomplete, response.body);
  await (0, import_promises3.rename)(incomplete, blobPath);
  await (0, import_promises3.symlink)(blobPath, pointerPath);
  return pointerPath;
}
...
async function snapshotDownload(params) {
  ...
  if (revision !== commitHash) {
    ...
    await (0, import_promises4.mkdir)((0, import_node_path3.dirname)(refPath), { recursive: true });
    await (0, import_promises4.writeFile)(refPath, commitHash);
  }
  ...
  for await (const entry of cursor) {
    switch (entry.type) {
      case "file":
        ...
        break;
      case "directory":
        await (0, import_promises4.mkdir)((0, import_node_path3.join)(snapshotFolder, entry.path), { recursive: true });
        break;
      default:
